...
@Function:
    Word 文件解析器 - 转 PDF 后复用 MinerU 解析链路

    策略：Word → LibreOffice headless 转 PDF → PDFParser(MinerU) 解析。
    Word 为流式排版，直接解析无可靠视觉 bbox；统一转 PDF 后可获得
    与 PDF 一致的精确坐标与版式结构。.doc / .docx 均走同一转换逻辑。
...
class WordParser(OfficeToPdfParser):
    ...
    功能：
    - 通过 LibreOffice headless 将 DOC/DOCX 转换为 PDF
