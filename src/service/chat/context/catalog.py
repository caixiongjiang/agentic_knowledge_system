...
2. 模型网关 ``/v1/model/info`` 上报值（经 LiteLLMRegistry 缓存，不触发网络）
...
class ModelContextCatalog:
    ...
    def __init__(
        ...
        *,
        ...
        self._long_context_path = long_context_path or _default_capability_path("long_context_models.json")
    ...
    def bare_name(model: str) -> str:
        raw = (model or "").strip()
        for prefix in (PROXY_PREFIX, "openai/"):
    ...
    def resolve(self, model: str) -> ModelContextSpec:
        ...
        if max_context is None or max_context <= 0:
            ...
            if source == "default" and bare not in self._warned_unknown:
                ...
                logger.warning(
                    f"[ModelContextCatalog] 未知模型 {bare!r}（本地未声明、网关未上报），"
                    f"使用默认 max_context={max_context}；"
                    f"若该模型窗口更小，请在当前档案的 long_context_models.json 中声明"
                )
    ...
    def _try_proxy_limits(bare: str) -> Optional[int]:
        """从 LiteLLMRegistry 的**已有缓存**取网关上报的上下文长度。
    ...
    def _try_litellm_limits(
        ...
        try:
            ...
            logger.debug(f"[ModelContextCatalog] litellm get_model_info 失败: {e}")
        return None, None
