...
- 专有名词 / 型号 / 代码片段等，可考虑是否加 `exact_match`；偏问答可思是否加 `qa_dense`；偏概览/结构可思是否加 `section_dense` / `section_summary_dense` / `file_summary_dense`；上下文重、需细粒度语义时可思 `enhanced_chunk_dense`——均为**可选启发**，由你判断。
...
2. **改写查询必须保持自然语言语义连贯**：
   - 对于 `chunk_dense` / `enhanced_chunk_dense` / `section_dense` / `qa_dense` / `section_summary_dense` / `file_summary_dense` 等**稠密向量路**，以及基于 BGE-M3 学习式稀疏向量的 **`bm25_sparse`**，改写后的 `query_text` **必须是一段自然语言句子或短语**，可以补充上下文/同义词/意图增强词（如"技术规格"、"参数表"、"操作步骤"），但**绝不要**改写成传统倒排 BM25 风格的"空格分隔关键词词袋"（如 `"型号A 参数1 参数2 参数3"`），否则会破坏语义编码效果。
...
def format_routes_description(descriptors: list) -> str:
    ...
    route_name_map = {
        ...
        "section_summary_vector_search": "section_summary_dense",
        "file_summary_vector_search": "file_summary_dense",
