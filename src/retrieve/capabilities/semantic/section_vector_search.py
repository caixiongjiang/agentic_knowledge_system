...
@File    : section_vector_search.py
@Author  : caixiongjiang
@Date    : 2026/02/28
@Function: 
    Section 粒度向量语义检索原子能力
@Modify History:
...
class SectionVectorSearch(BaseVectorSearch):
    """Section 粒度向量语义检索

    对 Milvus section_store Collection 执行 ANN 向量检索，
    返回与查询主题最相关的章节（Section）。

    对应 Collection: section_store
    对应 Repository: SectionRepository
    """
    ...
    def __init__(
        ...
        repository = SectionRepository(manager=milvus_manager)
        super().__init__(repository=repository, embedding_client=embedding_client)

    def _build_result_items(self, hits: List[Dict[str, Any]]) -> List[SectionItem]:
        items: List[SectionItem] = []
        for hit in hits:
            entity = hit.get("entity", {})
            items.append(SectionItem(
                section_id=str(hit.get("id", "")),
    ...
    def describe(self) -> CapabilityDescriptor:
        return CapabilityDescriptor(
            ...
            display_name="Section 向量主题检索",
            description=(
                "在 section_store 中执行向量 ANN 检索，"
                "召回与查询主题最相关的章节。"
                "适用于探索性提问、宏观主题概览。"
            ),
            input_schema={
            ...
            output_type="RetrieveResult[SectionItem]",
        )
