...
│  → knowledge_base:split:end → knowledge_base:summary:end → ...       │
│                                                                      │
...
graph LR
    subgraph 第一层-组件流转[第一层:组件流转]
        ...
        B[ImageUnderstand] -->|图片描述| T1
        C[TextAnalyzer] -->|summary/atomic_qa| T1
    end
...
graph TD
    ...
    subgraph 后台串行[后台阶段 - 串行]
        B --> F[FileSummary]
    end
    
    subgraph 后台并行1[后台阶段 - 第一层并行]
        F -->|knowledge_base:summary:end| G[KGExtractor]
        F -->|knowledge_base:summary:end| H[ImageUnderstand]
        
        G -->|triples| I[db_write:graph:start]
        H -->|图片描述| C
    end
    
    subgraph 后台串行2[后台阶段 - TextAnalyzer]
        H -->|knowledge_base:image:end| J[TextAnalyzer]
        J -->|summary/qa| C
    end
    
    style 前台阶段 fill:#e1f5e1
    style 后台串行 fill:#fff3cd
    style 后台并行1 fill:#e3f2fd
    style 后台串行2 fill:#fce4ec
```
...
| 依赖关系 | 原因 |
|---------|------|
| FileSummary 在 TextSplitter 后 | 需要chunk数据生成文件摘要 |
| KGExtractor 在 FileSummary 后 | KG抽取需要参考摘要作为上下文 |
| ImageUnderstand 在 FileSummary 后 | 图片理解需要参考摘要作为上下文 |
| KGExtractor 与 ImageUnderstand 并行 | 两者相互独立,可同时处理 |
| TextAnalyzer 在 ImageUnderstand 后 | 图片是chunk的一部分,需要先理解图片才能做完整的summary/qa |
...
graph TB
    ...
    subgraph 计算层[计算层 - Stateless Workers - 只做业务逻辑]
        ...
        subgraph 后台串行[后台串行组件]
            W_SUMMARY[FileSummary Worker<br/>LLM: 中]
        end
        
        subgraph 后台并行[后台并行组件]
            W_ANALYZER[TextAnalyzer Worker<br/>LLM: 中]
            W_KG[KGExtractor Worker<br/>LLM: 高]
            W_IMG[ImageUnderstand Worker<br/>VLM: 高]
        end
    end
    ...
    subgraph 消息层[消息层 - Kafka Cluster]
        ...
        subgraph 流转Topic[第一层: 任务流转 Topics]
            ...
            T_SUMMARY_DONE[knowledge_base:summary:end]
            T_KG_DONE[knowledge_base:graph:end]
            T_IMG_DONE[knowledge_base:image:end]
        end
    ...
    %% 后台串行:FileSummary
    T_SPLIT --> W_SUMMARY
    W_SUMMARY --> T_SUMMARY_DONE
    
    %% 后台并行:FileSummary后,KGExtractor和ImageUnderstand并行
    T_SUMMARY_DONE --> W_KG
    T_SUMMARY_DONE --> W_IMG
    
    %% KG完成事件
    W_KG --> T_KG_DONE
    
    %% ImageUnderstand后触发TextAnalyzer
    W_IMG --> T_IMG_DONE
    T_IMG_DONE --> W_ANALYZER

    %% 第二层:数据库写入连接(统一Key: user_id:file_id)
    W_SPLIT -->|原始文本chunk<br/>Key: user:file| T_DB_EMBED
    W_IMG -->|图片描述<br/>Key: user:file| T_DB_EMBED
    W_ANALYZER -->|summary/qa<br/>Key: user:file| T_DB_EMBED
    ...
    %% 监控层连接
    T_VEC_INDEXED & T_IMG_DONE & T_KG_DONE -.->|tap| MONITOR
    MONITOR --> REDIS
...
sequenceDiagram
    ...
    participant Summary as FileSummary
    participant Analyzer as TextAnalyzer
    participant KG as KGExtractor
    participant ImageW as ImageUnderstand
    ...
    Note over Summary: 后台串行阶段
    K->>Summary: 消费 knowledge_base:split:end
    Summary->>K: 发送 knowledge_base:summary:end
    end
    
    rect rgb(227, 242, 253)
    Note over KG,ImageW: 后台并行阶段(KGExtractor + ImageUnderstand)
    par KGExtractor
        K->>KG: 消费 knowledge_base:summary:end
        KG->>K: 发送 db_write:graph:start
        KG->>K: 发送 knowledge_base:graph:end
    and ImageUnderstand
        K->>ImageW: 消费 knowledge_base:summary:end
        ImageW->>K: 发送 db_write:embedding:start (图片描述)
        ImageW->>K: 发送 knowledge_base:image:end
    end
    end
    
    rect rgb(252, 228, 236)
    Note over Analyzer: 后台串行阶段(TextAnalyzer)
    K->>Analyzer: 消费 knowledge_base:image:end
    Analyzer->>K: 发送 db_write:embedding:start (summary/qa)
    end
...
2. **后台串行1**: FileSummary 监听 `knowledge_base:split:end`(生成摘要)
3. **后台并行**: KGExtractor 和 ImageUnderstand 同时进行(都依赖摘要结果)
4. **后台串行2**: TextAnalyzer(依赖图片理解结果,图片是chunk的一部分)
...
- knowledge_base:summary:end(文件摘要完成)
...
| `knowledge_base:split:end` | 32 | 触发多个Consumer(FileSummary, StatusManager) | 10-20 | LLM生成摘要 |
...
| Topic名称 | 分区数 | 设计依据 | 预期Consumer数 | 并发瓶颈 |
|-----------|--------|---------|---------------|---------|
| `knowledge_base:summary:end` | 16 | 后台任务,并行度要求低 | 10-30 | LLM抽取速度 |
| `knowledge_base:graph:end` | 16 | 状态同步,消息量小 | 2-5 | 仅StatusManager消费 |
| `knowledge_base:image:end` | 16 | 后台任务,承接KG完成 | 10-20 | TextAnalyzer处理速度 |
...
| Topic名称 | Partitions | Message Key | 分区策略 | 生产者 | 消费者组 | 消息保留 | 说明 |
|-----------|------------|-------------|---------|--------|----------|----------|------|
| `knowledge_base:summary:end` | 16 | `{user_id}:{file_id}` | Hash(key) % 16 | FileSummary | `group-kg-extractor`, `group-image-understand` | 3天 | 文件摘要完成,触发后台KG和图片理解 |
| `knowledge_base:graph:end` | 16 | `{user_id}:{file_id}` | Hash(key) % 16 | KGExtractor | `group-status-manager` | 3天 | 知识图谱抽取完成 |
| `knowledge_base:image:end` | 16 | `{user_id}:{file_id}` | Hash(key) % 16 | ImageUnderstand | `group-text-analyzer`, `group-status-manager` | 3天 | 图片理解完成,触发TextAnalyzer |
...
|-----------|------------|-------------|---------|--------|----------|-----------|------|
| `db_write:embedding:start` | 32 | `{user_id}:{file_id}` | Hash(key) % 32 | TextSplitter, ImageUnderstand, TextAnalyzer | `group-embedding-milvus-writer` | 100条/500ms | **核心**: 原始文本 → Embedding → Milvus |
| `db_write:graph:start` | 16 | `{user_id}:{file_id}` | Hash(key) % 16 | KGExtractor | `group-neo4j-writer` | 500条/2s | 图谱写入 |
| `db_write:meta:start` | 32 | `{user_id}:{file_id}` | Hash(key) % 32 | All Workers | `group-mysql-writer` | 200条/1s | 元数据写入 |
| `db_write:mongo:start` | 32 | `{user_id}:{file_id}` | Hash(key) % 32 | TextSplitter, ImageUnderstand | `group-mongo-writer` | 100条/500ms | 文档数据写入 |
...
| ImageUnderstand | 图片描述文本 | chunk_store_zh/en (type=image) |
| TextAnalyzer | summary/atomic_qa | summary_store_zh/en, atomic_qa_store_zh/en |
...
2. 加载所有文本Chunk
3. 调用LLM生成文件摘要
4. 发送 `knowledge_base:summary:end` 事件

### 4.4 KGExtractor Worker

```mermaid
graph LR
    A[knowledge_base:summary:end] --> B[KGExtractor Worker]
    B --> C{处理流程}
    C --> D[加载Chunk内容和摘要]
    C --> E[并发调用LLM抽取三元组]
    C --> F[统一抽取实体/关系/事件]
    C --> G[去重和清洗]
    B --> H[knowledge_base:graph:end]
    B --> I[db_write:graph:start]
```

**职责**:
- 统一抽取知识三元组(实体关系、事件实体、事件关系)
- 并发处理多个Chunk
- 三元组去重和清洗
- 发送图谱写入消息
...
1. 加载Chunk内容和文件摘要
2. 并发调用LLM抽取三元组
3. 统一抽取实体关系、事件实体、事件关系
4. 去重和质量过滤
5. 发送图谱写入消息

### 4.5 ImageUnderstand Worker

**职责**:
- 多模态图片理解
- 生成图片描述文本
- 为TextAnalyzer提供完整的chunk信息

**处理流程**:
1. 从S3加载图片chunk
2. 调用VLM(Vision Language Model)理解图片
3. 生成图片描述文本
4. 发送原始文本到向量化队列
...
**职责**:
- 生成chunk级别的summary和atomic_qa
- 依赖图片理解结果(图片是chunk的一部分)
- 使用LLM生成高质量分析

**处理流程**:
1. 加载文本chunk和图片描述
2. 调用LLM生成summary和atomic_qa
3. 发送原始文本到向量化队列
...
graph TD
    subgraph 生产者[所有需要向量化的组件]
        ...
        A2[ImageUnderstand] -->|图片描述| T1
        A3[TextAnalyzer] -->|summary/qa| T1
    end
...
graph TD
    subgraph Kafka Topics
        ...
        T4[knowledge_base:summary:end]
        T5[knowledge_base:image:end]
...
| `knowledge_base:summary:end` | - | 后台: 摘要生成完成 |
| `knowledge_base:image:end` | - | 后台: 图片理解完成 |
...
- FileSummary 监听 `knowledge_base:split:end` 事件,生成文件摘要
...
后台阶段不影响前台进度:
knowledge_base:summary:end:     None
knowledge_base:graph:end:       None
knowledge_base:image:end:       None
```
...
| 组件 | 实例数 | 资源配置 | 扩容触发条件 | 说明 |
|-----|--------|---------|-------------|------|
| FileSummary Worker | 10 | 4 CPU, 8GB RAM | Kafka lag > 50 | LLM生成摘要 |
| TextAnalyzer Worker | 10 | 4 CPU, 8GB RAM | Kafka lag > 50 | LLM分析(summary/qa),依赖图片理解 |

#### 8.1.3 后台并行 Workers

| 组件 | 实例数 | 资源配置 | 扩容触发条件 | 说明 |
|-----|--------|---------|-------------|------|
| KGExtractor Worker | 10 | 4 CPU, 8GB RAM | Kafka lag > 50 | 统一KG三元组抽取 |
| ImageUnderstand Worker | 10 | 4 CPU, 8GB RAM, 1 GPU | Kafka lag > 50 | VLM图片理解 |
...
┌──────────────────────────────────────────────────────────────────┐
│                    后台串行阶段1 - FileSummary                    │
│  knowledge_base:split:end → FileSummary                          │
│  → knowledge_base:summary:end                                    │
└──────────────────────────────────────────────────────────────────┘
                              ↓
┌──────────────────────────────────────────────────────────────────┐
│                    后台并行阶段                                   │
...
│        ↓                                                         │
│  ┌─────────────────────┐  ┌───────────────────────────────────┐ │
│  │ KGExtractor         │  │ ImageUnderstand                   │ │
│  │ 统一三元组抽取        │  │ 图片chunk理解                      │ │
│  └─────────────────────┘  └───────────────────────────────────┘ │
│        ↓                                   ↓                     │
│  db_write:graph:start         knowledge_base:image:end          │
└──────────────────────────────────────────────────────────────────┘
                                             ↓
┌──────────────────────────────────────────────────────────────────┐
│                    后台串行阶段2 - TextAnalyzer                   │
│  knowledge_base:image:end → TextAnalyzer                         │
│  → db_write:embedding:start (summary/qa,需要图片理解结果)        │
└──────────────────────────────────────────────────────────────────┘
```
...
12. **后台串行1**: FileSummary 监听 `knowledge_base:split:end`(生成摘要供后续组件使用)
13. **后台并行**: KGExtractor 和 ImageUnderstand 同时进行
14. **后台串行2**: TextAnalyzer 依赖 ImageUnderstand(图片是chunk的一部分,需先理解图片)
...
| TextSplitter | 原始文本chunk | chunk_store_zh/en |
| ImageUnderstand | 图片描述文本 | chunk_store_zh/en (type=image) |
| TextAnalyzer | summary/atomic_qa | summary_store, atomic_qa_store |
