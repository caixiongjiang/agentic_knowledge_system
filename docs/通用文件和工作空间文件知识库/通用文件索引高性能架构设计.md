...
|-----------|------------|-------------|---------|--------|----------|-----------|------|
| `db_write:embedding:start` | 32 | `{user_id}:{file_id}` | Hash(key) % 32 | TextSplitter, ImageUnderstand, TextAnalyzer | `group-db-writer` | 100条/500ms | **核心**: 原始文本 → Embedding → Milvus |
...
| `db_write:mongo:start` | 32 | `{user_id}:{file_id}` | Hash(key) % 32 | TextSplitter, ImageUnderstand | `group-db-writer` | 100条/500ms | 文档数据写入 |
