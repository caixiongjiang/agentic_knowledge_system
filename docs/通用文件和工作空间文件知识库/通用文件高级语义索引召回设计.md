...
QA 召回作为多路召回（`ParallelRecall`）的一条语义路由（`qa_dense`），与 `chunk_dense` / `enhanced_chunk_dense` / `section_summary_dense` / `file_summary_dense` 等路由并行执行，结果汇入统一候选池。**只有两档，没有直答短路**：
...
**两档分流（按 `qa_dense` top1 分数；θ_pin 默认 0.9，需实测校准）**：

1. `qa_dense` 与其他语义/词法路由**始终并行跑完**，不因高分提前结束。
2. **档 A（score < θ_pin）**：不把 `answer` 交给模型。该 QA（及未过阈值的其它 QA）只把 `source_chunk_ids` 展开成 chunk，注入候选池，与其它路一起对齐 → 融合 → 精排。工具只回精排 Top-K。
3. **档 B（score ≥ θ_pin 且 answer 非空）**：
   - **通道 1（置顶）**：把该 QA 的 `question` / `answer` / score 放在工具返回最前；同时把它**自己的** `source_chunk_ids`（通常 1–2 条，不是整个 section）从 Mongo `chunk_data` 补展示正文，作为「依据原文」紧随其后。这些依据 chunk **不进 reranker**。
   - **通道 2（照常精排）**：其它路结果仍走对齐 → 融合 → 精排。精排候选与最终 Top-K **剔除**已置顶的 `chunk_id`，避免重复。
   - 工具返回 = 置顶 QA + 依据原文 + 去重后的 Top-K。模型仍可用原文核对、补全。
4. `QAItem → ChunkItem` 的转换在 `ParallelRecall` 统一扁平化处完成（`chunk_id=f"qa:{qa_id}"` 仅作候选池占位，真正 chunk 溯源由 `source_chunk_ids` 承担）。

> 已删除原「档 C / 直答短路」：高分不再跳过 align/fusion/rerank，也不再只回一条 answer。wire 字段 `direct_answer` 保留，语义改为置顶 QA；`RecallStats.short_circuited` 恒为 False，另增 `qa_pinned`。

流程图见同目录 `qa-dense-two-paths.svg`。
...
- **检索侧集成**：QA 召回作为 `qa_dense` 路由接入 `ParallelRecall` 多路召回，与既有 chunk/summary 路由并行。高置信走置顶（θ_pin，默认 0.9）+ 依据 chunk 旁路；低分只把 `source_chunk_ids` 注入候选池。两档都在多路召回框架内实现，不另起独立召回链路，也不短路。
